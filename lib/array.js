...
inherits(ArraySchema, MixedSchema, {
  ...
  _cast(_value, _opts) {
    const value = MixedSchema.prototype._cast.call(this, _value, _opts); //should ignore nulls here
  ...
  _validate(_value, options = {}) {
    ...
    return MixedSchema.prototype._validate.call(this, _value, options).catch(propagateErrors(endEarly, errors)).then(value => {
      ...
      let validations = value.map((item, idx) => {
        var path = makePath`${options.path}[${idx}]`; // object._validate note for isStrict explanation
